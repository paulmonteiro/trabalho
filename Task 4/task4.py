...
with open('content3.json') as jsonfile:
...
for i in range(0,len(data)):
    ...
    for j in range(i+1,len(data)) :
        color2 = data[j]['color']
        previous = compatibles[-2]
        for k in range(0, len(data)):
            if data[k]['name'] == previous:
                previous_min_version=data[k]['min_version']
                previous_max_version=data[k]['max_version']
        if (color2 == color1) and (version_overlaps(data[i]['min_version'],data[i]['max_version'],data[j]['min_version'],data[j]['max_version'])) and (version_overlaps(data[j]['min_version'],data[j]['max_version'],previous_min_version,previous_max_version)):
